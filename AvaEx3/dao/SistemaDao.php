...
class SistemaDao {
    ...
    $sql = "SELECT s.*, 
                   pl.padrao_lancamento AS nome_padrao, 
                   pp.pacotes_padrao AS nome_pacote, 
                   d.derivado AS nome_derivado
            FROM sistemas s
            JOIN padrao_lancamento pl ON (pl.id = s.padrao_lancamento_id)
            JOIN pacotes_padrao pp ON (pp.id = s.pacotes_padrao_id)
            JOIN derivados d ON (d.id = s.derivado_id)
            WHERE s.id = ?";
...
public function update(Sistema $sistema) {
    try {
        ...
        $stm->execute(array(
            ...
            $sistema->getDesenvolvedora(),
            $sistema->getVersao(),
            $sistema->getPadraoLancamento() ? $sistema->getPadraoLancamento()->getId() : null,
            $sistema->getPacotesPadrao() ? $sistema->getPacotesPadrao()->getId() : null,
            $sistema->getDerivado() ? $sistema->getDerivado()->getId() : null,
            $sistema->getId()
        ));
    } catch (PDOException $e) {
        die($e->getMessage());
