...
if (isset($_POST['titulo'])) {
    ...
    if ($numPaginas == '') {
        array_push($erros, "Informe o Número de paginas!");
    }else if($numPaginas < 0){
        array_push($erros, "Número de paginas invalidas, deve ser maior que zero");
    }
    ...
    if (empty($erros)) {
        ...
        $msgErro = implode("<br>", $erros); //Faz o mensagens erros ser uma variavel com as strings espaçadas
    }
}
...
<body>
    ...
    <!--<form method="POST" onsubmit="return validar();"> --> <!--Versão validando com JS - Frontend-->
    <form method="POST">

        <input type="text" name="titulo" id="titulo" placeholder="Informe o título" value="<?= $titulo ?>" />
        <br><br>

        <input type="text" name="autor" value="<?= $autor ?>" placeholder="Informe o autor">
        <br><br>

        <select name="genero" id="genero">
            <option value="">--Selecione o gênero--</option> <!-- Usando operador ternario, damos uma condição, caso true e caso false -->
        ...
        <input type="number" name="numPaginas" value="<?= $numPaginas ?>" placeholder="Informe o número de páginas">
        <br><br>
    ...
    <table border="1">
        ...
        <?php foreach ($livros as $l): ?>
            <tr>
                ...
                <td><?= $l['paginas'] ?></td>
                <td>
                    <a href="excluir.php?id=<?= $l['id'] ?>" onclick="return confirm('Confirma a exclusão?')"> Excluir </a>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
